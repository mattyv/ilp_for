// ilp::find_if - godbolt example
// Vectorizable first-match search: compiler+ISA-resolved default (Clang: byte-scaled
// blockcheck; GCC 15+/SSE4.1+/AArch64: plain loop, deferring to GCC's own early-break
// vectorizer; else: SLP-safe blockcheck). See docs/PERFORMANCE.md for the measured
// tables behind these defaults.
...
// ============================================================================
// Example: first index where a uint32_t exceeds a threshold
// ============================================================================

// ilp::find_if - the dedicated vectorizable first-match primitive
std::size_t first_over_ilp(const std::vector<uint32_t>& data, uint32_t threshold) {
    auto it = ilp::find_if(data, [threshold](uint32_t v) { return v > threshold; });
    return static_cast<std::size_t>(it - data.begin());
}

// Hand-rolled blockcheck: the shape ilp::find_if's Clang default (and its
// non-plain-loop GCC fallback) compiles down to - a branch-free "does this
// block contain a match?" scan, then a scalar re-scan of the hit block.
std::size_t first_over_handrolled(const std::vector<uint32_t>& data, uint32_t threshold) {
...
// Simple loop - what GCC 15+ with SSE4.1 or better resolves ilp::find_if's
// default to directly (see ilp::detail::gcc_prefers_plain_loop above).
std::size_t first_over_simple(const std::vector<uint32_t>& data, uint32_t threshold) {
...
int main() {
    volatile std::size_t n = 1000;
    volatile uint32_t threshold = 500;

    std::vector<uint32_t> data(n);
    for (std::size_t i = 0; i < n; ++i) {
        data[i] = (i == n / 2) ? 1000 : static_cast<uint32_t>(i % 100);
    }

    auto r1 = first_over_ilp(data, threshold);
    auto r2 = first_over_handrolled(data, threshold);
    auto r3 = first_over_simple(data, threshold);
