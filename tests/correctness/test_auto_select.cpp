...
TEST_CASE("Auto-selecting reduce sum", "[auto][reduce]") {
    ...
    SECTION("Index-based sum") {
        int sum = ilp::reduce_simple_auto(0, (int)data.size(), 0,
            std::plus<>{},
            [&](int i) { return data[i]; });
...
TEST_CASE("Auto-selecting reduce range sum", "[auto][reduce][range]") {
    ...
    SECTION("Function version") {
        int sum = ilp::reduce_range_simple_auto(data, 0, std::plus<>{},
            [](int val) { return val; });
        REQUIRE(sum == expected);
    }
...
TEST_CASE("Different element sizes use different N", "[auto][optimal_N]") {
    ...
    SECTION("int8") {
        ...
        int8_t sum = ilp::reduce_range_simple_auto(data, int8_t{0}, std::plus<>{},
            [](int8_t x) { return x; });
        REQUIRE(sum == 100);
    }
    ...
    SECTION("int16") {
        ...
        int16_t sum = ilp::reduce_range_simple_auto(data, int16_t{0}, std::plus<>{},
            [](int16_t x) { return x; });
        REQUIRE(sum == 100);
    }
    ...
    SECTION("int32") {
        ...
        int32_t sum = ilp::reduce_range_simple_auto(data, int32_t{0}, std::plus<>{},
            [](int32_t x) { return x; });
        REQUIRE(sum == 100);
    }
    ...
    SECTION("int64") {
        ...
        int64_t sum = ilp::reduce_range_simple_auto(data, int64_t{0}, std::plus<>{},
            [](int64_t x) { return x; });
        REQUIRE(sum == 100);
    }
}
