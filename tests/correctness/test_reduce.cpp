...
TEST_CASE("Reduce with Break", "[reduce][control]") {
    SECTION("Range-based reduce with break stops correctly") {
        ...
        auto result = ilp::reduce_range<4>(data, 0, std::plus<int>(), [](auto&& val) -> std::optional<int> {
            if (val > 5)
                return std::nullopt;
            return val;
        });
    ...
    SECTION("Index-based reduce with break stops correctly") {
        auto result = ilp::reduce<4>(0, 100, 0, std::plus<int>(), [](auto i) -> std::optional<int> {
            if (i >= 10)
                return std::nullopt;
...
TEST_CASE("Simple Reduce", "[reduce]") {
    SECTION("Sum of 0 to 99") {
        auto result = ilp::reduce<4>(0, 100, int64_t{0}, std::plus<>{}, [](auto i) { return static_cast<int64_t>(i); });
    ...
    SECTION("Sum of a vector") {
        ...
        auto result =
            ilp::reduce_range<8>(data, int64_t{0}, std::plus<>{}, [](auto&& val) { return static_cast<int64_t>(val); });
...
TEST_CASE("Bitwise Reduce Operations", "[reduce][bitwise]") {
    SECTION("Bitwise AND reduction") {
        ...
        auto result = ilp::reduce_range<4>(data, 0xFFu, std::bit_and<>(), [](auto&& val) { return val; });
    ...
    SECTION("Bitwise OR reduction") {
        ...
        auto result = ilp::reduce_range<4>(data, 0u, std::bit_or<>(), [](auto&& val) { return val; });
    ...
    SECTION("Bitwise XOR reduction") {
        ...
        auto result = ilp::reduce_range<4>(data, 0u, std::bit_xor<>(), [](auto&& val) { return val; });
    ...
    SECTION("Bitwise AND with index-based loop") {
        auto result = ilp::reduce<4>(0, 10, 0xFFFFFFFFu, std::bit_and<>(), [](auto i) { return ~(1u << i); });
...
TEST_CASE("Cleanup loops with remainders", "[reduce][cleanup]") {
    ...
    SECTION("Unsigned int reduce SIMPLE with std::plus<> and N=4") {
        auto result = ilp::reduce<4>(0u, 10u, 0u, std::plus<>(), [](auto i) { return i; });

        REQUIRE(result == 45u);
    }

    SECTION("Int reduce SIMPLE with std::plus<> and N=1") {
        auto result = ilp::reduce<1>(0, 10, 0, std::plus<>(), [](auto i) { return i; });

        REQUIRE(result == 45);
    }

    SECTION("Index reduce with remainder") {
        auto result = ilp::reduce<4>(0, 11, 0, std::plus<>{}, [](auto) { return 1; });
    ...
    SECTION("Reduce with break in cleanup loop") {
        ...
        auto result = ilp::reduce_range<4>(data, 0, std::plus<>(), [](auto&& val) -> std::optional<int> {
            if (val == 9)
                return std::nullopt;
            return val;
        });
