...
TEST_CASE("4 levels nested index loops", "[nesting][deep]") {
    ...
    ILP_FOR(void, auto i, 0, 3, 4) {
        ILP_FOR(void, auto j, 0, 3, 4) {
            ILP_FOR(void, auto k, 0, 3, 4) {
                ILP_FOR(void, auto l, 0, 3, 4) {
                    count++;
                } ILP_END;
            } ILP_END;
        } ILP_END;
...
TEST_CASE("5 levels nested index loops", "[nesting][deep]") {
    ...
    ILP_FOR(void, auto a, 0, 2, 4) {
        ILP_FOR(void, auto b, 0, 2, 4) {
            ILP_FOR(void, auto c, 0, 2, 4) {
                ILP_FOR(void, auto d, 0, 2, 4) {
                    ILP_FOR(void, auto e, 0, 2, 4) {
                        count++;
                    } ILP_END;
                } ILP_END;
            } ILP_END;
...
TEST_CASE("Nested range loops", "[nesting][range]") {
    ...
    int sum = 0;
    ILP_FOR_RANGE(void, auto&& row, matrix, 4) {
...
TEST_CASE("3-level nested range loops", "[nesting][range]") {
    ...
    int sum = 0;
    ILP_FOR_RANGE(void, auto&& plane, cube, 4) {
        ILP_FOR_RANGE(void, auto&& row, plane, 4) {
            ILP_FOR_RANGE(void, auto&& val, row, 4) {
                sum += val;
            } ILP_END;
        } ILP_END;
...
TEST_CASE("Index outer, range inner", "[nesting][mixed]") {
    ...
    int total = 0;
    ILP_FOR(void, auto multiplier, 1, 4, 4) {
...
TEST_CASE("Range outer, index inner", "[nesting][mixed]") {
    ...
    int total = 0;
    ILP_FOR_RANGE(void, auto&& base, bases, 4) {
        ILP_FOR(void, auto i, 0, 5, 4) {
            total += base + i;
        } ILP_END;
    } ILP_END;
...
TEST_CASE("Different N at each nesting level", "[nesting][varied]") {
    ...
    ILP_FOR(void, auto i, 0, 5, 8) {    // N=8
        ILP_FOR(void, auto j, 0, 5, 4) {  // N=4
            ILP_FOR(void, auto k, 0, 5, 2) {  // N=2
                ILP_FOR(void, auto l, 0, 5, 1) {  // N=1
                    count++;
                } ILP_END;
            } ILP_END;
        } ILP_END;
...
TEST_CASE("Empty inner loop", "[nesting][edge]") {
    ...
    ILP_FOR(void, auto i, 0, 5, 4) {
        outer_count++;
        ILP_FOR(void, auto j, 0, 0, 4) {  // Empty!
            inner_count++;
        } ILP_END;
    } ILP_END;
...
TEST_CASE("Single iteration inner loops", "[nesting][edge]") {
    ...
    ILP_FOR(void, auto i, 0, 10, 4) {
        ILP_FOR(void, auto j, 0, 1, 4) {  // Only j=0
            ILP_FOR(void, auto k, 0, 1, 4) {  // Only k=0
                count++;
            } ILP_END;
        } ILP_END;
    } ILP_END;
...
TEST_CASE("Triangular nested iteration", "[nesting][pattern]") {
    ...
    ILP_FOR(void, auto i, 0, 10, 4) {
        ILP_FOR(void, auto j, 0, i, 4) {  // j depends on i!
            count++;
        } ILP_END;
    } ILP_END;
