...
TEST_CASE("Indirect array access", "[unusual][indirect]") {
    ...
    int sum = 0;
    ILP_FOR_RANGE_SIMPLE(auto&& idx, indices, 4) {
        sum += data[idx];
    } ILP_END;
...
TEST_CASE("Bit counting", "[unusual][bits]") {
    auto popcount = ILP_REDUCE_SUM(auto i, 0, 256, 4) {
...
TEST_CASE("Struct field access", "[unusual][struct]") {
    ...
    int x_sum = 0, y_sum = 0;
    ILP_FOR_RANGE_SIMPLE(auto&& p, points, 4) {
        x_sum += p.x;
        y_sum += p.y;
    } ILP_END;
...
TEST_CASE("Atomic-like increments", "[unusual][atomic]") {
    ...
    ILP_FOR_SIMPLE(auto i, 0, 1000, 4) {
        (void)i;
        counter++;
    } ILP_END;
...
TEST_CASE("Find last occurrence via backward step", "[unusual][reverse]") {
    ...
    int found = -1;
    bool done = false;
    ILP_FOR_STEP_SIMPLE(auto i, 6, -1, -1, 4) {  // 6, 5, 4, 3, 2, 1, 0
        if (!done && data[i] % 2 == 0) {
            found = i;
...
TEST_CASE("Reduce to pair", "[unusual][nonnum]") {
    ...
    auto result = ILP_REDUCE_SIMPLE(op, init, auto i, 0, 100, 4) {
