...
TEST_CASE("All_of pattern (inverted)", "[unusual][pattern]") {
    ...
    auto result = ILP_FOR_UNTIL_RANGE(val, data, 4) {
        return val % 2 != 0;  // Find first non-even
    } ILP_END_UNTIL;

    REQUIRE(!result.has_value());  // All even
}
...
TEST_CASE("Indirect array access", "[unusual][indirect]") {
    ...
    int sum = 0;
    ILP_FOR_RANGE_SIMPLE(idx, indices, 4) {
        sum += data[idx];
    } ILP_END;
...
TEST_CASE("Struct field access", "[unusual][struct]") {
    ...
    int x_sum = 0, y_sum = 0;
    ILP_FOR_RANGE_SIMPLE(p, points, 4) {
        x_sum += p.x;
        y_sum += p.y;
    } ILP_END;
...
TEST_CASE("Find first duplicate", "[unusual][pattern]") {
    ...
    REQUIRE(result.has_value());
    REQUIRE(*result == 3);  // Index 3 is first duplicate (value 2)
}
...
TEST_CASE("Find last occurrence via backward step", "[unusual][reverse]") {
    // Find last even number by going backwards
    std::vector<int> data = {1, 2, 3, 4, 5, 6, 7};

    int found = -1;
    bool done = false;
    ILP_FOR_STEP_SIMPLE(i, 6, -1, -1, 4) {  // 6, 5, 4, 3, 2, 1, 0
        if (!done && data[i] % 2 == 0) {
            found = i;
            done = true;  // Can't break in SIMPLE, use flag
        }
    } ILP_END;

    // First backward even number is 6 at index 5
    REQUIRE(found == 5);
}
...
TEST_CASE("Reduce to pair", "[unusual][nonnum]") {
    ...
    auto result = ILP_REDUCE_SIMPLE(op, init, i, 0, 100, 4) {
