...
unsigned sum_with_break_handrolled(unsigned n, unsigned stop_at) {
    ...
    for (; i + 4 <= n && !stopped; i += 4) {
        ...
        if (i < stop_at) sum0 += i; else { stopped = true; }
        if (i + 1 < stop_at) sum1 += i + 1; else { stopped = true; }
        if (i + 2 < stop_at) sum2 += i + 2; else { stopped = true; }
        if (i + 3 < stop_at) sum3 += i + 3; else { stopped = true; }
    }
    for (; i < n && !stopped; ++i) {
        if (i >= stop_at) break;
        sum0 += i;
