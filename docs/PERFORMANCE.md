...
Apple M2, Apple Clang 21, 10M elements, `-O3 -march=native`

| Loop Type | Simple | Pragma | ILP | Speedup |
|-----------|--------|--------|-----|---------|
| `ILP_FOR` with `ILP_BREAK` | 1.53ms | 1.52ms | 0.97ms | **1.58x** |
| `ILP_FOR` with `ILP_RETURN` | 1.71ms | 1.52ms | 0.96ms | **1.79x** |
| `ILP_FOR` with `ILP_CONTINUE` | 2.03ms | 1.74ms | 1.52ms | **1.33x** |
| `ILP_FOR_RANGE` with `ILP_BREAK` | 2.15ms | - | 0.96ms | **2.24x** |

Times are `cpu_time` (median of 8 repetitions) — this is a shared machine that runs
under background load, so wall-clock `real_time` is inflated and unstable while
`cpu_time` stays robust; the prior Clang 19 table uses the same metric, and the two
runs agree to within noise (no meaningful change from Clang 19 → 21). GCC 15 is omitted
here: Homebrew GCC can't compile against the macOS 26 SDK's `mach/message.h`, so
google-benchmark won't build — a toolchain/SDK incompatibility, not an ILP result.
