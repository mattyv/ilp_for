...
void process_left_channel(std::vector<int16_t>& stereo_data) {
    ILP_FOR_STEP_SIMPLE(i, 0uz, stereo_data.size(), 2uz, 4) {
...
int sum_alpha_channel(const std::vector<uint8_t>& rgba_data) {
    int sum = 0;
    ILP_FOR_STEP_SIMPLE(i, 3uz, rgba_data.size(), 4uz, 4) {  // Start at index 3 (alpha)
        sum += rgba_data[i];
    } ILP_END;
    return sum;
}
...
std::vector<int> downsample(const std::vector<int>& data, size_t factor) {
    ...
    ILP_FOR_STEP_SIMPLE(i, 0uz, data.size(), factor, 4) {
...
void scale_diagonal(std::vector<double>& matrix, size_t width, double scale) {
    ...
    ILP_FOR_STEP_SIMPLE(i, 0uz, max_idx, stride, 4) {
        matrix[i] *= scale;
    } ILP_END;
}
